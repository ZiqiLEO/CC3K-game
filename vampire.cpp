    ...
    if(cmd == "no") {
        ...
        if(en == nullptr) {
            ...
            if(en->halfling()) {
                ...
                random = rand() % 2;
                if(random == 0) {
                    int add_hp = en->attacked(*this);
                    // gain ph for successfully attack
                    if(en->get_hp() <= 0) {
                    ...
                    hp += add_hp; // gain hp for every successful attack
                    output += "Vampire regains 5 HP. ";
                } else {
...
string Vampire:: get_pc_name() {
    return string("Vampire");
}
