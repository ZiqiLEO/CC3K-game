...
void Goblin:: attacked(Orc &o) {
    damage = ceil(1.5 * ((100.0/(100.0+def)) * o.get_atk()));
    hp -= damage;
}


void Goblin:: next(Game &g) {
    damage = 0;
    output = "";
    ...
    if(cmd == "no") {
        ...
        if(g.floor[y-1][x] == '.'
           ...
           || g.floor[y-1][x] == '#') {
            ...
            y -= 1;
            g.floor[y][x] = '@';
        } else if(g.floor[y-1][x] == '\\') { // '\'
            throw string("upstair");
        } else if(g.floor[y-1][x] == 'G') {
            g.floor[y][x] = sym_before;
            sym_before = '.';
            y -= 1;
            g.floor[y][x] = '@';
        ...
        if(g.floor[y-1][x-1] == '.'
           ...
           || g.floor[y-1][x-1] == '#') {
            ...
            move = false;
            output += "Cannot move to North Wast. Please check if type in right command. ";
        }
        if(move){
            for(int i = y-1; i <= y+1; ++i){
                for(int j = x-1; j <= x+1; ++j) {
