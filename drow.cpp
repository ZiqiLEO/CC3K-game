...
void Drow:: next(Game &g) {
    ...
    if(cmd == "no") {
        ...
        bool found_p = false;
        for(int i = 0; i < s; ++i) {
            // posion effect magnified by 1.5, because PC is drow
            if(g.potions[i].x == xy[0] && g.potions[i].y == xy[1]) {
                if(g.potions[i].potion_name == "RH") {
                    hp += 1.5 * g.potions[i].effect;
                    if(hp >= maxhp) {
                        hp = maxhp;
                    }
                    output += "PC used a RH. ";
